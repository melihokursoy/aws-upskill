...
export async function GET(): Promise<NextResponse> {
  ...
  if (process.env.LOCAL_AUTH_BYPASS === 'true') {
    return NextResponse.redirect(new URL('/', process.env.NEXT_PUBLIC_APP_URL ?? 'http://localhost:3300'));
  }
  ...
  const expired = 'expires=Thu, 01 Jan 1970 00:00:00 GMT; path=/; secure; httponly; samesite=none';
  response.headers.append('Set-Cookie', `AWSELBAuthSessionCookie-0=; ${expired}`);
  response.headers.append('Set-Cookie', `AWSELBAuthSessionCookie-1=; ${expired}`);
