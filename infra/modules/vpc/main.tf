...
# Public route table — no inline routes, route defined separately as aws_route
# so Terraform has an explicit dependency edge: route deleted before IGW detached.
...
# Separate route resource gives Terraform visibility: during destroy, this route
# is deleted first, then the IGW can be detached cleanly without DependencyViolation.
...
# Private route table — no inline routes, route defined separately as aws_route.
